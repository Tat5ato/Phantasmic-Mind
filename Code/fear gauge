...
public class fearGaugeCalculator: MonoBehaviour {
	...
	private bool nectarFMincd = false;
	private bool medicineFMincd = false;
	...
	IEnumerator adchecker ()
	{
		if (playerInstance.antiDepression == true) {
			medicineFMin = 1;
			medicineFMincd = true;
			yield return new WaitUntil (() => playerInstance.medicine == false);
			medicineFMincd = false;
		}
	...
	void Update () {
		...
		if (medicineFMincd == false) {
			if (playerInstance.medicine == true) {
		...
		if (Insane == false) {
			...
			if (inSpookyArea == true) {
				...
				//--------------------------------
				//medicine and nectar are good for you
				if (medicineFMin == 1) {
				...
				if (playerInstance.nectar == false) {
					if (nectarFMincd == false) {
